...
public class AfterHandBrake implements ProcessVideoFolders {
    ...
    @Override
    public void setDatesFromFileCreateDate(Path folder) {
        List<ExifTags> tagsToRead = new ArrayList<>();
        tagsToRead.add(ExifTags.DATE_CREATION_DATE);
        List<File> files = getListOfFiles(folder);
        files.stream().filter(File::isFile).forEach(f -> {
            try {
                Map<ExifTags, String> readedData = edm.readExifData(tagsToRead, f);
                
                String readedValue = readedData.get(ExifTags.DATE_CREATION_DATE);
                

                ZonedDateTime zdt = DateUtils.fromStringToZonedDateTime(readedValue, DateFormats.EXIF_DATE_TIME_WITH_ZONE);
                LocalDateTime ldt = LocalDateTime.ofInstant(zdt.toInstant(), zdt.getZone()).plusHours(2);
                String valueToStore = DateUtils.fromLocalDateTimeToString(ldt, DateFormats.EXIF_DATE_TIME);
                
                EnumMap<ExifTags, String> map = new EnumMap<>(ExifTags.class);

                map.put(ExifTags.DATE_DATE_TIME_ORIGINAL, readedValue);
                map.put(ExifTags.DATE_MODIFY_DATE, valueToStore);
                map.put(ExifTags.DATE_CREATE_DATE, valueToStore);
                
                map.put(ExifTags.DATE_FILE_MODIFIED, readedValue);
                map.put(ExifTags.DATE_FILE_CREATE_DATE, readedValue);
                map.put(ExifTags.DATE_FILE_ACCESS_DATE, readedValue);
